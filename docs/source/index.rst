...
Neutron is a dynamically typed general-purpose programming language implemented in `Python 3 <https://www.python.org/>`_. It can be run using `CPython <https://www.python.org/downloads/>`_ (slow), `PyPy <https://pypy.org/>`_ (very fast) or `Nuika <https://nuitka.net/>`_ (fast?), depending on your needs.
